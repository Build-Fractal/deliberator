...
**Status:** Compliant — admitted via v4.0.0 tier-extraction (2026-05-07); all Batch-1 Provisional remediations closed 2026-05-11. **+1 pending v4.1.0 ratification** (Persistence Contract Discipline, see Provisional remediation plan tail row; binding only at ratification). XII closed 2026-05-09 by `linter/dead_infra.py` + CI wiring. V closed 2026-05-11 by cross-mode parametrization in `linter/test_output_contract.py`. XXII closed 2026-05-11 by per-channel vendoring gates in `.github/workflows/distribution-surface-integrity.yml`. XXVI closed 2026-05-11 by `engine/tests/test_mode_provider_matrix.py`. XXIV closed 2026-05-11 by `SAFETY.md` enumerating 10 named perimeters and their independent guards.
...
| Persistence Contract Discipline (v4.1.0, Tier 1 Principle II sub-clause) — **PENDING RATIFICATION** | Deliberation-output parse contract lives in display text (`## Verdict` heading in `arbitration/resolution.md`; mode-specific dispute markers in 6 of 8 modes — pinned strict-xfail by the V remediation); no declared structured schema for the per-run artifact tree; `linter.output_contract` consumed cross-product (spec-kit-orc adapter) without a declared stability surface. | Implement structured XML/JSON-Schema output for deliberation phases + CI gate enforcing the declared schema. `CONSUMER-CONTRACT.md` draft staged on `spec/v4.1.0-impl-staging`; `schema/output-contract.md` placeholder staged. **Contract becomes binding only at v4.1.0 ratification.** Follow-on component-tier spec: `v4.1.1-conversus-structured-output`. | 2026-09-01 | `specs/v4.1.0-persistence-contract-discipline/spec.md` |
