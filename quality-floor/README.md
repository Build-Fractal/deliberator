...
Reference artifacts and test fixtures for validating that conversus multi-agent deliberation output is structurally differentiated from single-model responses.
...
3. **A worked side-by-side example** comparing conversus output to single-model responses
4. **Conversus configs and input questions** used to generate the reference outputs
5. **Single-model baseline responses** for the same questions as controls

These artifacts serve two purposes:
- **S01 (Quality Floor & Worked Example):** Empirically validate the conversus product premise — that 2-agent cooperative output is structurally differentiated from single-model responses.
...
├── configs/                            # conversus.yml configs used to generate outputs
...
- **[worked-example.md](worked-example.md)** — Side-by-side comparison with 6 annotated structural differences between conversus output and single-model responses.
