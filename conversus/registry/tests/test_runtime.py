"""Tests for :mod:`conversus.registry.runtime` (spec 064.1).
...
harness in the private ``conversus-enhanced`` repo.
...
from conversus.registry import runtime as runtime_module
...
def _make_capability(
    ...
    handler: str = "conversus.registry.tests.test_runtime:_stub_handler",
...
def test_register_mcp_uses_conversus_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert kwargs["name"] == "conversus_foo"
...
def test_resolve_handler_raises_on_missing_attr() -> None:
    with pytest.raises(AttributeError):
        runtime_module._resolve_handler(
            "conversus.registry.tests.test_runtime:does_not_exist"
        )
