...
Generated SKILL.md shape (for a capability named ``hello``)::
    ...
    - For full help: `conversus hello --help`
...
from conversus.registry.adapters.base import PluginAdapter
...
class DefaultPluginAdapter(PluginAdapter):
    ...
    def _title(self, cap: "Capability") -> str:
        heading = "Conversus " + cap.name.replace("-", " ").replace("_", " ").title()
    ...
    def _troubleshooting(self, cap: "Capability") -> str:
        return (
            "## Troubleshooting\n\n"
            f"- For full help: `conversus {cap.name} --help`"
        )
...
def _cli_invocation(cap: "Capability", params: list["Param"]) -> str:
    """Render a representative ``conversus <cap> ...`` shell invocation."""
    parts = [f"conversus {cap.name}"]
    for param in params:
        if param.required:
