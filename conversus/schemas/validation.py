...
Generates a conversus.yml configuration that launches a validation
...
def generate_validation_config(
    ...
    """Generate a conversus config dict for validating a solver solution.

    The returned dict can be written as YAML to produce a ``conversus.yml``
    ...
    Returns:
        Dict suitable for ``yaml.dump()`` to produce a valid ``conversus.yml``.
    ...
    A ``conversus validate`` CLI command will wrap this function, accepting
