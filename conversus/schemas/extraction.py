"""Feature extraction pipeline for conversus deliberation artifacts (spec 015).
...
parsing of the predictable conversus template output format.
...
_DISPUTES_BEGIN = re.compile(
    r"<!--\s*CONVERSUS:DISPUTES_BEGIN\s*-->", re.IGNORECASE,
)
_DISPUTES_END = re.compile(
    r"<!--\s*CONVERSUS:DISPUTES_END\s*-->", re.IGNORECASE,
)
...
def extract_features(
    ...
    """Extract feature vectors from a conversus deliberation output directory.
    ...
    Args:
        output_dir: Path to the conversus output directory (e.g. specs/X/conversus/).
