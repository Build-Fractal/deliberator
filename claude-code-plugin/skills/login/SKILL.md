---
description: Authenticate with a conversus model provider via OAuth. Walks through browser-based login for Anthropic, OpenAI, or other OAuth-capable providers.
---

# Conversus Login

Authenticate with a model provider so conversus can use it. Supports OAuth PKCE for providers that offer it (Anthropic, OpenAI, others).
...
Expected: `/conversus:login <provider>`
...
> `<provider>` uses an environment variable, not OAuth. Set `<PROVIDER_API_KEY>` in your environment instead, then run `/conversus:status` to verify.

## Step 2: Warn about the interactive flow

**Important**: `conversus login` requires interactive input. The browser-based OAuth flow opens a URL, the user authenticates in the browser, and the CLI prompts for an authorization code to paste back.
...
> If you already have an `ANTHROPIC_API_KEY` (or equivalent) as an environment variable, you don't need to log in at all — conversus picks it up automatically. Run `/conversus:status` to confirm.

## After login

Once the user completes login in their terminal, have them run `/conversus:status` to verify the credentials are stored, then `/conversus:decide` or `/conversus:run` to use the provider.
