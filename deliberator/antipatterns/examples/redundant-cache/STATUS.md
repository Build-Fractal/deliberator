...
**Depends On**: `001-subject-arbitration` (hard dependency for `/deliberator arbitrate`), `002-recursive-rounds` (for rounds > 1 in `/deliberator converge`), `004-preset-agents` (soft dependency for `/deliberator interests` preset suggestions)
...
**Risk-of-Gap**: Users must manually configure deliberator.yml for every deliberation; no guided workflow, interest discovery, or mode selection assistance.
...
**Discovery (FR-022–024)**: `/deliberator presets` list, filter, detail commands — not started. Becomes prerequisite when spec 003 Phase A enters development.
...
**Risk-of-Gap**: Agent definitions copy-pasted across deliberator configs; no reuse mechanism for common agent archetypes. Discovery of available presets requires filesystem browsing.
**Effort**: Small (discovery only) — 3 FRs for `/deliberator presets` list, filter, and detail commands. Core engine already complete.
...
**Changes required**:
- New "Subcommand Dispatch" section before Step 1 — routes `/deliberator define`, `/deliberator interests`, `/deliberator mode`, `/deliberator converge`, `/deliberator arbitrate` to their respective execution flows
- Entry point routing logic to distinguish subcommand invocations from `/deliberator run`
- Phase A/B/C orchestration within the `converge` subcommand
- Integration with preset resolution (spec 004) for `/deliberator interests` preset suggestions
...
003 Decision Framework ──→ 001 (hard: /deliberator arbitrate)
                           002 (rounds > 1 in /deliberator converge)
                           004 (soft: /deliberator interests preset suggestions)
...
- **004 next**: Self-contained engine with no upstream dependencies. Completing 004 before 003 enables preset suggestions in spec 003's `/deliberator interests` command.
