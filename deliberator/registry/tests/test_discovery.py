"""Tests for :mod:`deliberator.registry.discovery`.
...
from deliberator.registry import discovery as discovery_module
from deliberator.registry import __all__ as registry_all
...
def test_single_capability_via_modes_group() -> None:
    """One entry point under deliberator.modes → one Capability."""
    cap = make_capability("test_mode")
    ep = make_ep("test_mode", "deliberator.modes", lambda: cap)
    with patch_entry_points({"deliberator.modes": [ep]}):
...
def test_single_capability_via_solvers_group() -> None:
    ...
    ep = make_ep("test_solver", "deliberator.solvers", lambda: cap)
    with patch_entry_points({"deliberator.solvers": [ep]}):
        result = collect_capabilities()
    assert len(result) == 1
...
def test_capability_returned_directly_is_accepted() -> None:
    ...
    ep = make_ep("direct", "deliberator.presets", cap)
    with patch_entry_points({"deliberator.presets": [ep]}):
        result = collect_capabilities()
    assert len(result) == 1
...
def test_multi_group_composition() -> None:
    ...
    ep_mode = make_ep("alpha", "deliberator.modes", lambda: cap_mode)
    ep_solver = make_ep("beta", "deliberator.solvers", lambda: cap_solver)

    with patch_entry_points({
        "deliberator.modes": [ep_mode],
        "deliberator.solvers": [ep_solver],
    }):
        result = collect_capabilities()
...
def test_failure_load_time_exception_skips_entry(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="deliberator.registry.discovery"):
        with patch_entry_points({"deliberator.modes": [bad_ep]}):
            result = collect_capabilities()
...
def test_failure_wrong_return_type_skips_entry(caplog: pytest.LogCaptureFixture) -> None:
    ...
    ep = make_ep("wrong_type", "deliberator.modes", lambda: "not-a-capability")
    with caplog.at_level(logging.WARNING, logger="deliberator.registry.discovery"):
        with patch_entry_points({"deliberator.modes": [ep]}):
            result = collect_capabilities()
    assert result == []
...
def test_failure_factory_raises_skips_entry(caplog: pytest.LogCaptureFixture) -> None:
    ...
    ep = make_ep("explosive", "deliberator.solvers", boom)
    with caplog.at_level(logging.WARNING, logger="deliberator.registry.discovery"):
        with patch_entry_points({"deliberator.solvers": [ep]}):
            result = collect_capabilities()
    assert result == []
...
def test_failure_does_not_break_other_entries() -> None:
    ...
    good_ep = make_ep("survivor", "deliberator.modes", lambda: good_cap)
    bad_ep = make_ep("bad", "deliberator.modes", lambda: 42)  # int, not Capability

    with patch_entry_points({"deliberator.modes": [bad_ep, good_ep]}):
        result = collect_capabilities()
...
def test_all_snapshot() -> None:
    """``deliberator.registry.__all__`` matches the snapshot file exactly.
...
def test_walk_order_reflected_in_results() -> None:
    ...
    ep_mode = make_ep("z_mode", "deliberator.modes", lambda: cap_mode)
    ep_solver = make_ep("a_solver", "deliberator.solvers", lambda: cap_solver)
