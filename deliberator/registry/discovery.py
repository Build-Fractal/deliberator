...
(``deliberator.modes``, ``deliberator.presets``, ``deliberator.domains``,
``deliberator.solvers``) on each invocation and returns a fresh,
...
Composition is the caller's responsibility::
    ...
    from deliberator.registry import collect_capabilities
...
def collect_capabilities() -> list[Capability]:
    ...
    :class:`~deliberator.registry.capability.Capability` instance, or to
    a :class:`~deliberator.registry.capability.Capability` instance
