...
Generated tool shape::
    ...
    @mcp.tool()
    def deliberator_hello(who: str, loud: bool = False) -> str:
        \"\"\"Say hello\"\"\"
        from deliberator.demo import say_hello
...
from deliberator.registry.adapters._helpers import literal, split_handler
from deliberator.registry.adapters.base import MCPAdapter
...
#: Prefix applied to every generated MCP tool name. ``decide`` → ``deliberator_decide``.
#: Keeps the deliberator namespace separate from other MCP servers Claude may have.
TOOL_PREFIX = "deliberator_"
...
class DefaultMCPAdapter(MCPAdapter):
    ...
    def _signature(self, params: list["Param"], tool_name: str) -> str:
        """Emit ``def deliberator_<name>(...) -> str:``.
        ...
        hand-written ``deliberator_decide`` in ``mcp_server.py``.
        """
        if not params:
