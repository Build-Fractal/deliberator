"""Config parsing for the ``plugins:`` section of deliberator.yml.
...
class PluginConfigEntry(BaseModel):
    """A validated plugin declaration from deliberator.yml.

    Example YAML::

        plugins:
          - name: my-plugin
            package: deliberator-nashopt
            config:
              threshold: 0.8
    """
...
def parse_plugins_config(
    ...
    """Parse and validate the ``plugins:`` section from a deliberator config.
