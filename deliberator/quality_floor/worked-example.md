# Worked Example: Deliberator Output vs Single-Model Response

## Introduction

This document compares the output of a 2-agent deliberator cooperative deliberation against a single-model Claude response for the same questions. The goal is to identify **structural differences** — properties present in the multi-agent output that are structurally impossible in a single-model response, not just stylistically different.
...
For each, a 2-agent deliberator deliberation (pragmatist + devils-advocate) produced a synthesis through 5 phases (review → cross-review → revision → disputes → synthesis). A single Claude response answered the same question directly.
...
### Deliberator Synthesis (key excerpts)
...
The **Remaining Disputes** section, delimited by `DELIBERATOR:DISPUTES_BEGIN` / `DELIBERATOR:DISPUTES_END` markers, contains 3 named disputes:
...
**What deliberator has:** The synthesis contains formally structured disputes — sections delimited by `DELIBERATOR:DISPUTES_BEGIN` / `DELIBERATOR:DISPUTES_END` machine-parseable markers, each headed by `**Dispute: [Label]**`, each presenting two named agents' positions with their supporting arguments, a neutral synthesizer assessment, and a recommended resolution.
