"""Package-aware path resolution for deliberator data files.
...
When pip-installed, templates and presets live inside the ``deliberator``
...
Promoted from the private `deliberator` repo 2026-04-18 to fix a latent
broken-wheel bug: OSS imported ``from deliberator.paths import ...`` in
``engine/config.py``, ``engine/_root.py``, ``engine/templates.py``,
``linter/validate.py``, ``deliberator/schemas/construction.py``, and
``deliberator/schemas/game_forms.py`` -- but the module did not exist
in OSS. Published wheels would have failed on any code path that hit
those imports. See
``payer-index-mono/deliberations/deliberator-divergence/private-decomposition-audit.md``
...
def resolve_package_path(package: str, *parts: str) -> Path:
    ...
    Args:
        package: Top-level package name (e.g. ``"deliberator"``).
    ...
    if parts:
        current = Path(__file__).resolve().parent  # deliberator/
    ...
    raise FileNotFoundError(
        ...
        "Ensure the deliberator package is properly installed or run from "
        "the source tree."
    )
...
def get_templates_dir() -> Path:
    ...
    """
    return resolve_package_path("deliberator", "templates")
...
def get_template_path(mode: str, template_name: str) -> Path:
    ...
    """
    return resolve_package_path("deliberator", "templates", mode, template_name)
...
def get_presets_dir() -> Path:
    ...
    """
    return resolve_package_path("deliberator", "presets")
...
def get_scaffold_dir(domain_package: str, *subpath: str) -> Path:
    ...
    its module.  E.g.::
        ...
        get_scaffold_dir(
            "deliberator.domains.implementations.code_review",
            "scaffolds",
        )
