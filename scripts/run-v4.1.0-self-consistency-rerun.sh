#!/usr/bin/env bash
#
# run-v4.1.0-self-consistency.sh — orchestrated runner for the v4.1.0 SELF-CONSISTENCY
...
# Stage 3 (blind verification) will live at:
#   deliberations/v4.1.0-persistence-contract-discipline-blind-<DATE>/
#
...
DELIB_DIR="deliberations/v4.1.0-persistence-contract-discipline-self-consistency-rerun-2026-05-12"
...
# Provider auth check
# claude-code provider routes through Claude Code's own auth; no separate login.
# The originating-stage rate-limit crash (Phase 5 RateLimitError after 3 retries)
# was the same OAuth budget at the model layer, but a fresh run resets the
# per-minute budget window, so re-running this script after waiting 1-2 minutes
# is the documented mitigation.
say "Using $PROVIDER provider (claude-code) — no separate auth check required."
ok "Provider configured"

# Output dir cleanup
hdr "Cleanup partial state"
PARTIAL_DIRS=(strict-reader purist tier-coherence-auditor precedent-auditor summary arbitration)
cleaned=0
...
spec: self-consistency verification v4.1.0 outputs

Stage 2 RE-RUN (after DEMOTE-TO-TIER-2) per spec 067 verification protocol.
