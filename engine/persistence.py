...
logger = logging.getLogger("deliberator.persistence")
...
_DELIBERATOR_DIR = Path(".deliberator")
_DELIBERATIONS_REL = _DELIBERATOR_DIR / "deliberations"
_SETTINGS_FILE = _DELIBERATOR_DIR / "settings.yml"
...
def find_user_project_root(start: Path | None = None) -> Path:
    """Discover the user's deliberator *workspace* root.

    This is the directory that holds (or should hold) the user's
    ``.deliberator/`` folder — i.e. the place where deliberation output,
    ...
    the deliberator *package install root* (the directory containing
    ...
    1. ``DELIBERATOR_ROOT`` environment variable — set by the Desktop
    ...
    2. Walk up from *start* (or CWD) looking for ``.deliberator/`` or
       ``deliberator.yml`` — the standard marker-based discovery for the
       user workspace.
    ...
    for parent in [current, *current.parents]:
        if (parent / _DELIBERATOR_DIR).is_dir():
            return parent
        if (parent / "deliberator.yml").is_file():
            return parent
...
def is_persistence_enabled(project_root: Path) -> bool:
    ...
    resolves the full cascade: project ``.deliberator/settings.yml`` →
...
def persist_deliberation(
    ...
    ``<project_root>/.deliberator/deliberations/<timestamp>-<slug>/output/``
    and copies the full *source_dir* tree into it.  A ``question.md``
    file is written alongside ``output/``.  If *config_path* is
    provided, it is copied as ``deliberator.yml`` into the deliberation
    directory.

    The ``.deliberator/deliberations/`` directory is created lazily if it
    does not already exist.

    Args:
        source_dir: The engine output directory to persist.
        project_root: The project root containing ``.deliberator/``.
        question: The deliberation question text.
        config_path: Optional path to the ``deliberator.yml`` used for the
    ...
    if config_path is not None and config_path.exists():
        shutil.copy2(str(config_path), str(delib_dir / "deliberator.yml"))
...
def cleanup_old_deliberations(
    ...
    Scans ``<project_root>/.deliberator/deliberations/``, parses the
...
def list_deliberations(project_root: Path) -> list[dict[str, Any]]:
    ...
    Scans ``<project_root>/.deliberator/deliberations/`` and for each
    directory extracts the timestamp, reads ``question.md``, and reads
    ``deliberator.yml`` for mode and agent names.
    ...
    for child in delib_root.iterdir():
        ...
        mode: str | None = None
        agents: list[str] = []
        config_file = child / "deliberator.yml"
...
def read_deliberation_file(
    ...
    ``<project_root>/.deliberator/deliberations/`` to prevent path
    traversal attacks.
