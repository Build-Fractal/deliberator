<!--
Sync Impact Report
Version change: 2.5.0 → 2.6.0 (MINOR — Principle XVI Option A in-place
refactor per spec 070. Closes the v2.4.0 grandfathering gap for XVI by
elevating the structural substrate (parameter pinning + shape
determinism + plain-language output pairing) as the load-bearing
claim. The pre-v2.6.0 headline ("the user MUST understand what is
being optimized, even without understanding the math") was the
specific clause that failed Constitutional Inclusion Criterion 1
(mechanical verification): "user understanding" is not mechanically
checkable. The structural substrate that already lived in the body
IS mechanically checkable, and v2.6.0 makes that substrate the
headline claim while preserving the design intent as a derived
property.)
Added principles: none
Modified principles:
  - XVI. Mathematical Transparency — headline rewritten to elevate
    three structural requirements (parameter pinning, shape
    determinism, plain-language output pairing) as the load-bearing
    claim; opening paragraph restructured to position the original
    "user understands what is being optimized" framing as the design
    *intent* that the structural requirements achieve, rather than
    the mechanically-unverifiable claim itself. The 3-stage pipeline
    block, Clarification (v2.3.2) determinism-scope block,
    Observability sub-bullet, Enforcement sub-bullet, Falsification
    sub-bullet, and Origin note are all UNCHANGED — they constitute
    the structural substrate the new headline now elevates.
Removed sections: none
Templates requiring updates: none

Constitutional Inclusion Criteria self-assessment for the rewritten
Principle XVI (per the v2.4.0 gate in Governance):

  - Criterion 1 (Mechanical verification capability): PASS.
    (a) Parameter pinning is enforced by spec 014 FR-012/SC-004
    (`objective.yml` persistence) plus the CI lint detecting re-entrant
    `GapFiller.fill()` calls (already filed as a follow-up to spec 068).
    (b) Shape determinism is enforced by a parity test that asserts
    the assembled objective function is byte-identical given a fixed
    template and fully-pinned parameter set.
    (c) Plain-language output pairing for plugin recommendations is
    enforced by a structural lint on plugin recommendation output
    schemas (every numerical field paired with a plain-language
    explanation field, validated at registry-emission time per
    Principle XV's registry-as-extension-interface clarification).
    Each check is concretely sketchable in one paragraph, satisfying
    the gate's standard.

  - Criterion 2 (Falsifiable scope): PASS. Each of the three
    structural requirements is specific enough to flag a violating
    PR without interpretation:
      - A PR that re-invokes `GapFiller.fill()` mid-deliberation is
        a parameter-pinning violation (mechanical lint).
      - A PR whose template-assembly output diverges across two
        runs with identical pinned parameters is a shape-determinism
        violation (parity-test failure).
      - A PR that adds a numerical recommendation field without an
        adjacent plain-language explanation field is a pairing
        violation (schema lint).
    No reviewer reasoning of the form "X might be okay if Y" is
    needed for any of the three.

  - Criterion 3 (Distinctness): PASS. This is a refining-in-place
    amendment per Criterion 3's refinement provision ("Refining or
    extending an existing principle goes in that principle's body,
    not as a new principle"). The structural substrate — parameter
    pinning, shape determinism, plain-language output pairing —
    covers ground no other principle covers:
      - Principle VII (Reproducibility) covers structural-output
        reproducibility but explicitly carves out LLM gap-filling
        per the v2.3.2 bilateral carve-out; XVI fills that carve-out
        with the pinning discipline.
      - Principle VIII (Templating Engines Over Inference) governs
        template-driven behavior in general; XVI specializes it to
        objective-function assembly with the shape-determinism
        guarantee.
      - Principles IX, XI, XV, XXII, XXIV all touch adjacent
        concerns (typing, single-source-of-truth, plugin isolation,
        distribution integrity, defense-in-depth) but none of them
        codify the parameter-pinning + shape-determinism + plain-
        language-pairing triad that defines mathematical transparency
        for optimization-driven decisions.
...
Rationale: Per spec 070 §4.3's PASS WITH FIXES verdict on the v2.5.0
grandfathered-principle audit, Principle XVI's pre-v2.6.0 headline
("the user MUST understand what is being optimized, even without
understanding the math") fails Constitutional Inclusion Criterion 1:
"user understanding" is not mechanically checkable, and the
v2.4.0 grandfathering disclosure flagged XVI as one of three pre-gate
principles failing under the new criteria. However, the spec 070
audit also surfaced that the *structural substrate* already living
in XVI's body — the 3-stage pipeline, the v2.3.2 determinism-scope
clarification with its `objective.yml`-anchored pinning discipline,
the spec 014 FR-012/SC-004 enforcement cross-reference, and the
plain-language plugin-recommendation requirement — IS mechanically
verifiable, falsifiable, and distinct. Option A (this amendment)
rewrites the headline to elevate that substrate as the load-bearing
claim, retaining XVI's constitutional status while closing the
grandfathering gap.

The design *intent* — that users understand what is being optimized
without understanding the math — is preserved as a derived property
in the new opening paragraph: it is what the three structural
requirements achieve, not the constitutional claim itself. The
constitution tracks the mechanically-verifiable structural ground;
the design intent stays in the prose as the principle's animating
purpose.
