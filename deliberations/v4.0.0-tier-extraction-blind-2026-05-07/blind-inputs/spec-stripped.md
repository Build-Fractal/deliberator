...
- **Cross-reference rewrites — patterns that actually appear in the current constitution (resolves Fix #7):**
  ...
  Backward references (higher tier → lower tier, e.g., Tier 1 → Component) DO appear in current text in a few governance-section cross-references; they're rare but the linter's cross-reference resolution check (§6.8) verifies them generically.
...
- Add a Sync Impact Report header documenting the v4.0.0 ratification, with cross-references to the originating deliberation (`../conversus-oss/deliberations/v4.0.0-tier-extraction-originating-(date redacted)/`) and the spec at `../conversus-oss/specs/v4.0.0-tier-extraction/`.
...
#### Check (a) — Cross-tier duplication detection

**Algorithm:** for each component-tier principle in `conversus-oss/CONSTITUTION.md`, compute a signature consisting of:
...
Both lines are passed through normalization: collapse multiple whitespace runs to single spaces; strip leading/trailing whitespace per line. Then compare the normalized signature against every Tier 1 and Tier 2 principle's normalized signature. **FAIL** if any match.

**Scope rationale:** header + first paragraph is sufficient because principle bodies vary heavily in length (some are 5 lines, some are 200), but their headers and opening paragraphs are stable identification surfaces. Comprehensive duplication of a relocated principle would inevitably duplicate the header and at least the opening sentence. Full-body matching would produce false positives on shared boilerplate (Origin attributions, Extension blocks).
...
|---|---|---|
| Empirical templating-surface audit | DISCHARGED (date redacted) | conversus_enhanced/ has zero templating code per grep. VIII Universal classification stands. |
| Plugin Isolation post-PR-#30 verification | DISCHARGED (date redacted) | conversus/ local dir contains only self-audit/ + self-review/ (deliberation archives, NOT a Python package). (prior PR redacted) framework deletion verified clean. |
...
## 10. Implementation order — dependency-ordered + atomic
...
- Self-consistency verification (§8.1) PASS or PASS-WITH-FIXES with all fixes applied.; 7 fixes applied in this v2.)
...
Originating deliberation:
  deliberations/v4.0.0-tier-extraction-originating-(date redacted)/
...
This spec is **(under review)**. To proceed:

1. Run self-consistency verification per §8.1.
2. Run blind verification per §8.2 — markers stripped via `scripts/strip-constitution-for-blind.py`.
3. If blind passes (or override-with-rationale logged): open implementation PR per §10.
...
5. On merge: status flips from `Draft v2` to `Ratified — v4.0.0`.

If blind verification surfaces ACCEPT-level findings on new content, address findings (in a v3 spec) or override per §8.3 before re-running.
