...
Detailed decision records with full rationale are in [`decisions/`](decisions/).

### Q1: Plugin output in report — [DECIDED](decisions/Q01-plugin-output-in-report.md)

**Decision**: Deterministic append to end of `summary/final.md`. Plugins write structured YAML to `plugins/{name}.yml` with both machine-readable `data` and structured `display` fields (headline, body, recommendation, severity). Each plugin ships its own narrative template. The engine renders and appends a `## Plugin Insights` section. Without plugins, the section is absent — clean FR-005 compliance.

### Q2: Convergence predictor stop authority — [DECIDED](decisions/Q02-convergence-predictor-stop-authority.md)

**Decision**: Recommend-then-confirm. On stagnation prediction, the engine pauses and prompts the user to cancel or continue. No arbiter routing, no config suggestions at the pause point. Evolves to auto-cancel (opt-out via `auto_stop: false`) once prediction accuracy is proven over sufficient runs.

### Q3: Config optimizer in decision framework — [DECIDED](decisions/Q03-config-optimizer-in-decision-framework.md)

**Decision**: Mode-dependent. In interactive mode (`/conversus mode`), the optimizer presents recommendations with reasoning and the user confirms. In auto mode (`/conversus run` with `auto_optimize: true`), recommendations are silently applied. Default is advisory-only; auto-apply requires explicit opt-in.

### Q4: Pricing model — [DEFERRED](decisions/Q04-pricing-model.md)

**Status**: Research complete, decision pending. Local-only architecture (Q5) rules out usage-based and credit models. Sidekiq model (private PyPI, credential-gated downloads, per-org pricing) is the leading candidate. See decision record for full market research including Sidekiq, AG Grid, Nx Powerpack, Gurobi/CPLEX, and developer tool pricing trends.

### Q5: Data collection — [DECIDED](decisions/Q05-data-collection.md)

**Decision**: Local-only. No telemetry. No data leaves the machine. Convergence predictor works on current-run data. Config optimizer ships pre-trained. Per-user run history improves local predictions over time. Population-level training is a good problem to have if adoption reaches that scale.

### Q6: Objective function templates — [DECIDED](decisions/Q06-objective-function-templates.md)

**Decision**: Curated for v1. Ship ~29 canonical objective function forms, mathematically validated against known equilibria and tested against real deliberation data. The library proves conversus-nashopt works before opening to contributions. Long-term: accept community templates via PR with mathematical review.

### Q7: Scenario storage — [DECIDED](decisions/Q07-scenario-storage.md)

**Decision**: File-based. `scenarios/{id}.yml` in the project directory, git-tracked. Database when there's a concrete need, not before.

### Q8: Gap-filling model — [DECIDED](decisions/Q08-gap-filling-model.md)

**Decision**: User-configurable via `gap_fill_model` in plugin config, default to cheapest capable model. Gap-filling is structured translation (parse gaps → ask questions → map answers to parameters), not reasoning.

### Q9: Embedding model — [DECIDED](decisions/Q09-embedding-model-selection.md)

**Decision**: Bundled default (top MTEB model), user-overridable in plugin config. Users with domain-specific needs (medical, legal) can swap the model.

### Q10: Scenario sharing — [DECIDED](decisions/Q10-scenario-sharing.md)

**Decision**: No sharing infrastructure. Scenarios are YAML files (Q7); git handles distribution. Marketplace is a future product decision.
