...
**Revised by**: storyteller
**Date**: 2026-04-03
**Cross-reviews addressed**: technical-writer, docs-updater

## Changes made

### Post 1
1. **"60-line" -> "single-module resolver"** -- Dropped the line count entirely per both reviewers. The module is 164 lines; saying "60" was wrong. Now says "a single-module path resolver" which is accurate regardless of future changes.
2. **"41 specs" -> "43 specs"** -- Technical-writer flagged the done directory has 43 entries. Corrected the opening and the section header.
3. **"48 hours" -> "two calendar days"** -- Clarified this is wall-clock time, not effort hours. Technical-writer noted ambiguity.
4. **Added `scenarios.toml` and `swe.toml`** -- Technical-writer noted Post 1 only showed core.toml and solvers.toml. Added a paragraph noting the full four-package structure.
5. **Kept narrative strengths** -- Arc labels (monolith/portable/monetizable), hook opening, "no nag screens" cadence all preserved per synthesis instructions.

### Post 2
6. **Added three-tier solver fallback** -- Technical-writer noted Post 2 never mentions AMPL or nashopt tiers. Added a section after the eight-mode table explaining the heuristic -> AMPL/HiGHS -> nashopt/JAX fallback chain, framed as "what the paid tier adds on top of the pattern."
7. **Toned down tunability claims** -- Docs-updater flagged that only `gamma` exists today for PD mode. Added an explicit caveat that the parameter surface is currently one knob, with expansion planned. Removed implication of broad per-mode tunability.
8. **Softened Kalman convergence claim** -- Docs-updater noted this is specced but not in user docs yet. Changed from a concrete capability description to "the architecture supports" language, with a note that it ships with the solvers package but is not yet documented for end users.
9. **Kept narrative strengths** -- Opening hook ("you run three agents..."), four-step actionable pattern, imperative closing cadence all preserved.
...
This is the story of how three waves of work -- executed over two calendar days -- took conversus from monolith to portable to monetizable.

<!-- more -->

## The Starting Point: What 43 Specs Built
...
Or the prisoners-dilemma payoff, which computes `territory_held - gamma * overreach_penalty`. That `gamma` parameter is currently the only user-tunable knob across all eight modes -- it controls how harshly agents are penalized for overreach in PD mode. Set it high and agents are punished for crossing boundaries. Set it low and they get away with more. The payoff function is pure: same features, same gamma, same score every time. No randomness, no prompt sensitivity, no model variance.
...
The fix was `conversus/paths.py`, a single-module path resolver that finds data files using `importlib.resources` as the primary strategy and `Path(__file__)` traversal as a fallback for development. The key function is `resolve_package_path`:
...
Two more packages round out the set: `packages/scenarios.toml` builds `conversus-scenarios` (the scenario runner plugin and domain implementations), and `packages/swe.toml` builds `conversus-swe` (software engineering domain templates and workflows). Each follows the same pattern -- `only-include` the relevant directories, declare their own dependencies, and share no code with the other premium packages.
...
`pip install conversus-solvers` adds the quantitative layer. Equilibrium scoring, convergence tracking, config optimization, and all eight payoff functions. These are the features that make agent output consistent, comparable, and reproducible.
...
**Monetizable** (post-Wave 3): `pip install conversus` is a real thing. Four packages -- core, solvers, scenarios, swe -- each built from the same source tree. The free/paid boundary is drawn, tested, and enforced by the build system. The development workflow is unchanged -- same repo, same test command, same import paths.
...
description: "AI agents are non-deterministic by nature. Every run produces different output. Optimization models give you the scoring layer to make that output consistent, comparable, and reproducible."
---
...
## Beyond Heuristics: The Three-Tier Solver Fallback

The payoff functions described above are heuristic scorers. They are fast, interpretable, and good enough for most use cases. But they are not the ceiling.

The scoring architecture supports three tiers, each more powerful than the last:

**Tier 1: Heuristic payoffs.** The pure functions above. They run in milliseconds, require no external dependencies, and produce the `(payoff, best_response_payoff)` pair directly from extracted features. This is what ships with the free tier's scoring interface.

**Tier 2: AMPL/HiGHS linear programming.** For modes that map to formal optimization problems -- resource allocation maps to linear programming, fair division maps to envy-free allocation -- AMPL formulations with the HiGHS solver can compute provably optimal allocations. The gap between the heuristic score and the LP-optimal score tells you exactly how much value was left on the table. Install with `pip install conversus-solvers[ampl]`.

**Tier 3: nashopt/JAX game-theoretic solvers.** For modes where Nash equilibrium is the right solution concept -- adversarial, negotiation, mechanism design -- the nashopt plugin uses JAX to compute actual equilibrium strategies via gradient-based optimization. This is the full game-theoretic analysis: not "how close to equilibrium did the agents get?" but "here is the equilibrium, and here is how far each agent deviated from it."

Each tier falls back gracefully. If nashopt is not installed, the system uses AMPL. If AMPL is not installed, it uses heuristics. The heuristic tier always works. The higher tiers add precision, not functionality.

This fallback chain is what makes the scoring layer practical. You start with heuristics, validate that the pattern works for your use case, and upgrade to formal solvers when you need provable optimality.
...
**Step 4: Track across rounds.** If your system runs multiple rounds, feed scores into a simple filter (exponential moving average works). When the score stabilizes, you've converged. When it diverges, stop early and save the tokens.
